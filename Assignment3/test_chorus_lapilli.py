...
# =========================== [ADD YOUR TESTS HERE] ===========================
    ...
    def test_turn_alternation(self):
        # Click first tile (should be X)
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        tiles[0].click()
        self.assertTileIs(tiles[0], self.SYMBOL_X)

        # Click a different tile (should be O)
        tiles[4].click()
        self.assertTileIs(tiles[4], self.SYMBOL_O)

    def test_no_move_on_filled_tile(self):
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        # First click on tile[0] sets it to X.
        tiles[0].click()
        self.assertTileIs(tiles[0], self.SYMBOL_X)
        # Click the same tile again.
        tiles[0].click()
        # The tile should remain unchanged.
        self.assertTileIs(tiles[0], self.SYMBOL_X)
    ...
    def test_specific_move_sequence(self):
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        tiles[1].click()
        self.assertTileIs(tiles[1], self.SYMBOL_X)
        tiles[4].click()
        ...
        tiles[3].click()  # Clicking an already filled tile should not change it
        self.assertTileIs(tiles[3], self.SYMBOL_X)
        
        # The last move attempts to click tile 8
        tiles[8].click()
        
        # Ensure tile 8 is still BLANK (indicating move rejection due to game state)
